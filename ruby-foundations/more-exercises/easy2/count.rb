# The Enumerable#count method iterates over the members of a collection, passing each element to the associated block. 
...
def count(*list)
  count = 0
  list.each { |item| count += 1 if yield(item)}
  count
end
